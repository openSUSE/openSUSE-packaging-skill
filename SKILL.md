...
- `bug-scan.sh` — open bugzilla bugs for one package (the "investigate bugs when you touch it" hard rule); REST fallback — prefer the bugzilla MCP server's `bugs_quicksearch` tool when your harness has it connected (recommended server: **bugwarden**, policy-guarded — setup + migration in `references/bugzilla-cve-triage.md` §6).
