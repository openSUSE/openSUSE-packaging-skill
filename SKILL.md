...
Every runnable script prints usage with `-h`/`--help` (`_anitya.py` is a module, not a command); the user-scoped ones (my-packages, my-requests, sr-status, preflight) default the OBS account to `osc whois` unless `--user` is given.
