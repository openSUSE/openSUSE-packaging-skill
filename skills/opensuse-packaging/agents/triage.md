...
2. **Find candidates** with `scripts/outdated.py` (Repology "outdated in Tumbleweed" ∩ the set). Treat every hit as a *candidate*. **Then read its closing `# COVERAGE:` line and its exit code**: 3 means a source could not answer (or a pass never ran), so the sweep is partial — names only that source could have flagged were never checked. Never report "nothing needs updating" from such a run.
