...
2. **Find candidates** with `scripts/outdated.py` (Repology "outdated in Tumbleweed" ∩ the set). Treat every hit as a *candidate*. **Then read its closing `# COVERAGE:` line and its exit code**: 3 means a source could not answer (or a pass never ran), so the sweep is partial — a `WARNING:` on stderr names each down source and the run carries on with the rest — names only that source could have flagged were never checked. Never report "nothing needs updating" from such a run.
...
**Output contract:** a short report grouping candidates into **(a) likely-real updates** (with current→target and the upstream date), **(b) intentional pins / multi-track — skip** (with the reason), **(c) Repology artifacts / downgrades — skip**, **(d) devel already ahead** — in flight (report the existing SR id, no action) or stranded (forward-only: `osc sr <devel> <pkg> openSUSE:Factory` — no Block-2 dispatch), and **(e) NOT CHECKED — a source was down** (name the source and the affected names, taken from the `# COVERAGE:` line and the per-source `UNREACHABLE` / `Source0 registry is down` lines. Only the `UNREACHABLE` and `Source0 registry is down` lines belong here. The other two `# forge:` lines are **not** coverage gaps and must stay out of (e): `the source answered with no usable release` (tagless or absent upstream — it answered) and `no forge resolvable from URL:/Source0:` (nothing to ask; these need a by-hand or cadence check, report them under (b) with that reason); an empty (e) bucket is a claim that every source answered, so only write one when it did). Do **not** edit specs or build here — hand the likely-real list back to the orchestrator, which drives Block 2 (`agents/update-build.md`) per package.
