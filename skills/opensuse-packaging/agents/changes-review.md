...
**A Leap `pool/` tree (a `leap-16.x` branch) — pick the review by what the tree is.** `git diff --quiet origin/factory HEAD` exits 0 for a verbatim content-sync of Factory's pool branch:
- **Verbatim sync → the scoped Leap checklist**, not items 1–9: the content already passed Factory's review, and what is new is the target. Each is a BLOCKER:
  1. **BuildRequires resolve on every arch the PR builds.** `target-gate.sh --build` goes red unless every arch the PR project builds resolves; its GREEN line names the arch it built and the ones that resolved. Without that output, resolve each arch of `osc repos openSUSE:Backports:SLE-16.x:PullRequest` (where the PR bot builds — 16.1's builds fewer arches than Backports) with `osc buildinfo --alternative-project openSUSE:Backports:SLE-16.x standard <arch> <pkg>.spec`.
  2. **Companion `Provides` exist in the target.** Every `Requires:` the sync adds or changes — a virtual symbol above all — must have a provider in Backports or the SLFO base, whose spec there carries it (`osc cat PRJ PKG PKG.spec`); a missing one fails only in openQA, after the PR is green → `leap-slfo.md` "A sync can need a COMPANION package".
  3. **The soname / consumer effect.** A sync that moves a SONAME or drops a subpackage breaks what already links it in the target: `scripts/rdeps.sh <lib> openSUSE:Backports:SLE-16.x` lists those consumers, and each needs a rebuild or its own sync, named in the PR body.
- **Hand-edited tree (anything else) → the full checklist above, plus a mandatory BuildRequires diff against Factory's spec:** `diff <(git show origin/factory:<pkg>.spec | grep -E '^(Build)?Requires') <(grep -E '^(Build)?Requires' <pkg>.spec)`. Every differing line needs a reason in the Leap target; a dependency Factory changed and the Leap tree kept is a BLOCK. (Real case: tesseract-ocr 5.5.3 builds its man pages with asciidoctor; the hand-edited Leap spec kept `BuildRequires: asciidoc`, and the pushed tree failed at `%install`.)

**Verdict (output contract):** in a git checkout the first line names the tree you reviewed — `PASS — tree <sha12> — <reason>` or `BLOCK — tree <sha12>`, `<sha12>` being the first 12 characters of `git rev-parse HEAD^{tree}` — so the verdict cannot be carried to another tree. For a pool PR the caller records the verdict file with `target-gate.sh <clone> --review FILE`, which reads only that first non-empty line — `PASS` and HEAD's `tree <sha12>` must both be on it; a tree quoted further down (Factory's, say) is ignored, and HEAD's tree named only below it is red. Review the committed tree, not uncommitted edits.
