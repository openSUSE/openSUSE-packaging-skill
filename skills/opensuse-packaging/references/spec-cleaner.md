...
- `-p` / `--pkgconfig` — convert deps to `pkgconfig(...)` (**always pass**). NOTE: `-p` is *pkgconfig*, **not** perl. **Whenever a spec has any `pkgconfig(...)` dependency, make sure `BuildRequires: pkgconfig` is also present** — pkg-config is needed at build time, and although it's usually pulled in transitively, it must be declared explicitly. spec-cleaner injects it automatically when it adds `pkgconfig(...)` deps; if you add `pkgconfig(...)` deps by hand, add `BuildRequires: pkgconfig` yourself, and verify it's there after any cleanup.
  - **`--perl` can be *destructive* on specs that are not Perl packages but carry `BuildRequires: perl`**: it explodes the single `perl` dep into hundreds of `perl(...)` provider BRs and has been seen to **silently drop an adjacent unrelated `BuildRequires` line** (real case: a JS-runtime spec lost `git-core`, breaking the build). On non-Perl packages, if a `--perl` pass produces a large BR explosion, drop the flag for that package and record the deviation in a spec comment; the other three flags stay mandatory.
  ...
  - **More over-expansion footguns spec-cleaner commits — REJECT these and keep the package-name form (accepted no-diff deviations):**
    ...
    - **`--tex` can explode a *multi-file* texlive package into individual `tex(file.ext)` BuildRequires that don't all resolve.** `texlive-latex`, `texlive-amsmath`, `texlive-fancyhdr`, etc. each bundle dozens of `.cls`/`.sty`/`.def`/`.tex` files together; spec-cleaner converts the one package-name dep into one `tex(...)` line per file it can find — and some of those names (e.g. `tex(source2e.tex)`, `tex(utf8-test.tex)`, `tex(utf8test.tex)`) are upstream TeX Live source/test artifacts that no installed package actually provides, so `osc build`'s buildinfo comes back `unresolvable: nothing provides tex(source2e.tex)` and the build never starts. `zypper se --provides "tex(name.ext)"` resolving for a handful of spot-checked names is **not** sufficient proof — the failure only shows up for the specific unlucky names, so the real gate is a green local build, not a per-name resolve check. Keep the texlive-*package-name* form for any texlive package that bundles multiple files; only accept a per-file `tex(...)` swap when that upstream package genuinely maps 1:1 to one file (e.g. `texlive-footnotehyper` → `tex(footnotehyper.sty)`, which is what it ships). (Real case: sdcc 4.6.0.)
    In all five the spec is **not** no-diff (spec-cleaner re-suggests the expansion every run) — that's an accepted deviation; verify the *rest* of the diff is empty and move on.
  ...
  - **Beware macro-ification *inside a `sed`/`grep` pattern*, and of any literal upstream path spec-cleaner mistakes for one of ours.** spec-cleaner rewrites path-looking text anywhere it finds it, including a literal string you are *matching* in upstream content, or a literal directory name from the extracted source tree that merely happens to spell a macro's expansion — e.g. `sed -e '1s|^#!/usr/bin/env node$|…|'` becomes `#!%{_bindir}/env node`, or `install -Dpm 0644 cmd/foo/etc/bar.service …` (upstream's own `cmd/foo/etc/` source directory, nothing to do with our `%{_sysconfdir}`) becomes `install -Dpm 0644 cmd/foo%{_sysconfdir}/bar.service …`. Both expand to the same bytes today, but the line now reads as a path *we* emit rather than upstream text we're matching or upstream's own tree layout, and both silently break if the macro's expansion ever diverges from the literal (or if the sed pattern is meant to match text that predates the macro existing). Keep the literal; accepted deviation. (Real cases: typescript-go's typescript7 shebang fixup; syncthing's `cmd/strelaysrv/etc/linux-systemd/` install source; gnu_parallel's `#!/usr/bin/env perl` shebang-fixup sed pattern.)
  - **Beware bracing a *parameterized* macro — `%{macro} arg` is not the same call as `%macro arg`.** `%macro arg` expands the macro *with* `arg` as its parameter; `%{macro} arg` expands the (now zero-argument) macro first and leaves `arg` as unrelated trailing literal text. spec-cleaner sometimes wraps a bare macro invocation in braces as if it were a plain value macro (`%name` → `%{name}`, always safe) without checking whether the macro actually consumes the rest of the line as an argument. On a parameterized macro this is a real break, not a style nit: `%systemd_user_preun %{name}.service` → `%{systemd_user_preun} %{name}.service` fails at spec-parse time with `error: The %systemd_user_preun macro requires some arguments`, before the build even starts. Keep the bare `%macro arg` form for any macro that takes one (`%systemd_user_preun`/`%systemd_user_postun`/`%systemd_post`/`%service_add_post` and friends, `%patch -P N`, `%setup -n …`, etc.) and treat the brace-wrap as an accepted deviation. (Real case: syncthing 2.1.5 — cost a full build cycle to diagnose.)

**Documenting one of these deviations: never write the broken syntax literally.** RPM macro-expands *everywhere*, not just executable spec sections — including `#`-comments (`warning: Macro expanded in comment`, but it still evaluates) and the free-text body of `%changelog` entries, with no warning at all there. A spec comment or `.changes` bullet that literally quotes the offending sequence (`` `%{systemd_user_preun}` rewrite breaks... ``) gets macro-expanded by RPM the same as a real invocation, reproducing the exact parse error the comment was written to explain — one line earlier than the actual bug, which makes it more confusing to diagnose, not less. Describe the deviation in prose instead ("wrapping systemd_user_preun in braces breaks argument passing" — no `%`/`%{}` around the macro name at all); if you must show the literal syntax, escape every `%` as `%%`. Check both the spec comment *and* the matching `.changes` entry — the same session can (and did) hit this twice, once in each file. (Real case: syncthing 2.1.5 — the spec-comment version was caught by a local build, the `.changes`-entry version by the adversarial reviewer.)
