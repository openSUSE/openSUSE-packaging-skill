...
The exact usage of every bundled script: don't run `--help` — CI checks this file against each script's real flags and exit codes. Why each exists and the traps it encodes: `scripts/README.md`.
...
- `scm-snapshot.sh <git-url> [--rev SHA|BRANCH] [--base X.Y.Z] [--pkg NAME] [--update]` — `--update` re-pins `./_service` in place; exit 0 verified · 1 mismatch · 2 usage or a step failed
...
- `changes-prepend.sh <name>.changes [--author 'Name <email>']` — bullets on stdin; exit 0 verified · 1 verification failed (restored) · 2 usage or no author
- `changes-lint.sh [--entries N | --all] <file>.changes [...]` — exit 0 clean · 1 findings · 2 usage
- `changes-guard.sh [--base FILE] [--amend-top AUTHOR] <pkg>.changes [...]` — exit 0 insertion-only · 1 prior entry changed · 2 usage
...
- `incoming-requests.py [--user U] [--format ascii|table|plain] [--verbose] [--no-prs]` — exit 0 listed · 2 OBS query failed
- `watch-submissions.sh [--user U] [--login NAME] [--state-dir DIR] [--allow-empty] [--no-prs] [--no-incoming]` — first line BASELINE-INIT, NOCHANGE, CHANGED or WATCH-ERROR; exit 0 · 2 WATCH-ERROR or usage
- `autoforward-gate.sh <project> <package> [--user U] | --batch FILE` — exit 0 ELIGIBLE · 3 BLOCKED · 4 NOT_YOURS · 5 unreadable · 2 usage
- `devel-of.sh <package> [target-project]` — exit 0 prints devel/pkg · 3 not in target · 4 no devel project · 2 usage
- `cone-status.sh <project> [repo] [arch]` — exit 0 all green · 1 in flight · 2 settled failure

## Leap

- `leap-status.sh <pkg>` — exit 0 in sync · 1 behind, no PR · 2 behind, PR open · 3 not in Leap · 5 network
