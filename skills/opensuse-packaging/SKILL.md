...
1. a clean local `osc build` — `scripts/build-summary.sh`'s exit code *is* the verdict; **for a Leap pool branch, `scripts/target-gate.sh --build` (or `--remote`) in the pool clone itself — a green build of any other checkout is not evidence**;
...
**HARD RULE — a Leap `pool/` PR is opened or updated only by `scripts/pool-pr.sh`**, which refuses unless `target-gate.sh` stamped that exact tree GREEN on the PR's base and its review PASS; **never merge a pool PR, not even your own**. It is done when `sr-status.py --pr pool/<pkg>#<n>` exits 0. → `references/leap-slfo.md` "The pool PR gate — build, review, open, watch"
...
- `sr-status.py` — the Block-3 watch view: OBS SRs *and* Gitea PRs in one table, declines first. `--pr pool/<pkg>#<n>` reads that PR's OBS build at its head — the bot's last comment, or a green build of an older commit, is not the verdict.
...
- `outdated.py` — Repology ∩ your package set, plus an Anitya pass and a forge pass over what Repology cannot see. **A source being down degrades the run, never aborts it** — read the closing `# COVERAGE:` line; a lost source means the sweep is NOT a clean bill of health.
...
- `pr-guard.py` — the harness hook that refuses pool PR creates, merges and head pushes outside `pool-pr.sh`; the harness runs it, never an agent.
